...
const Testimonial = () => {
  return (
    <section className="relative w-full flex flex-col justify-between items-start mt-20">
      ...
      <span className="flex items-center gap-x-2 mt-4 text-white text-center">
        <button role="navigation" className="bg-primary w-8 h-8 p-1 text-xs rounded-full border border-line"><i><AiOutlineArrowLeft /></i></button>
        <p className="text-[0.7rem] text-offwhite">1<span className="text-line">/1</span></p>
        <button role="navigation" className="bg-primary w-8 h-8 p-1 text-xs rounded-full border border-line"><i><AiOutlineArrowRight /></i></button>
      </span>
    </section>
  );
};
